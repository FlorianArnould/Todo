...
public class EditProjectActivity extends AppCompatActivity implements OnObjectLoadedListener<Project> {
	...
	private void updatePriorityProperty() {
		setOnPropertyClickListener(R.id.priority_property, v -> new DialogManager(EditProjectActivity.this).showPriorityPickerDialog(_project.getPriority(), result -> {
	...
	private void updateDeadlineDateProperty() {
		setOnPropertyClickListener(R.id.deadline_date_property, v -> new DialogManager(EditProjectActivity.this).showDatePickerDialog(_project.getDeadline(), result -> {
	...
	private void updateDeadlineWholeDayProperty() {
		...
		setOnPropertyClickListener(R.id.deadline_whole_day_property, v -> checkBox.setChecked(!checkBox.isChecked()));
	}

	private void updateDeadlineTimeProperty() {
		setOnPropertyClickListener(R.id.deadline_time_property, v -> new DialogManager(EditProjectActivity.this).showTimePickerDialog(_project.getDeadline(), result -> {
			Calendar calendar = Calendar.getInstance();
			if (_project.hasDeadline()) {
				calendar.setTime(_project.getDeadline());
